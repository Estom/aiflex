...
class ContextCompressor:
    ...
    async def compress(
        ...
        target_count = max(
            self.min_messages,
            int(len(messages) * self.compression_ratio),
        )
    ...
    async def _compress_messages(self, messages: list[ChatMessage]) -> ChatMessage:
        ...
        Returns:
            ChatMessage: 压缩后的摘要消息
        """
        # 构建对话记录文本
        transcript = self._build_transcript(messages)

        # 构建提示词
        prompt = CONTEXT_COMPRESSION_PROMPT.format(transcript=transcript)
        ...
        try:
            ...
            return ChatMessage(
                role="system",
                content=f"[历史对话已省略，共 {len(messages)} 条消息]",
            )
